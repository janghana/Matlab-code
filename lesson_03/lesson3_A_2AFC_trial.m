...
% The goal in this lesson is to generate code to run a two-alternative forced-choice trial
% on a field of partially coherent moving dots.
% The subject will decide after a stimulus whether the overall motion of the dots was upward or downward.

%% the 'movingDots' function

% Just like on a TV cooking show, I've written a useful function based on some of the ideas
% in Lesson 2 that we can pull out of the oven that animates moving dot stimuli.
% You can see how to use it by typing:

% help movingDots

%% movingDots.m can put up multiple fields of moving dots along with a fixation spot.
% Each field can have its own speed, direction, color, lifetime, aperture size and location,
% size of dots and 'coherence' (more on coherence later).

% Here's an example of two fields of overlapping dots on one side of the screen -
% one red moving upward and one green moving downward.
...
% changing the 'coherence' of the field, which is the proportion of dots moving 
% in a particular direction. If there are 100 dots with direction 0 (up)0 and
% the coherence is set to 0.25, then 25 of the dots will move upward and
% the other 75 will be given random directions uniformly distributed 
% from 0-360 degrees.

% Here's an example of a single field of 15% coherent white dots in the center
% of the screen.
...
% Did you see the weak upward motion? 15% coherence should be near your threshold
% for detecting motion.
%% The 'drawFixation' function

% Notice the fixation spot. This was drawn by my function 'drawFixation.m'.
% It draws a box-shaped fixation point on top of a circular mask on every frame at the center of the screen. You can see how to use it by getting help:
...
% The default values were used in the demo above.
% You can change them by changing values the display structure like this:
...
% In order to get a subject's response, we need to read the keyboard.
% The best way to do this is with the PsychToolbox's 'KbCheck' function and
% its related friends.
...
% The second is a value related to the time the key was first pressed,
% and the third is an integer that is a
...
% Please press a key.
% You pressed the "u" key after  1.99 seconds
% How does this work? The program goes into the 'while' loop and spins around until the variable
% 'keyIsDown' changes to anything but zero as the result of a key press.
% When this happens, the control flows out of the loop where the key is decoded and RT is calculated.
% The PsychToolbox function 'GetSecs' returns a high-precision clock time,
% where zero is arbitrary - it's values are useful only with respect to other calls to GetSecs.

% Here's a somewhat silly example using GetSecs.
% The clock is first read and stored as variable 'a'.
% The program then pauses for one second using Matlab's 'pause' function,
% and the clock is read again but stored into the variable 'b'.
% The values of a and b are basically meaningless on their own,
% but their difference reflects the time between calls to GetSecs:
...
% Time Elapsed: 1.0063 seconds
% You might notice that the time, b-a, is not exactly equal to 1.00 seconds.
% This discrepancy is due to inaccuracy with 'pause', not 'GetSecs'.
...
% amount of time. This is useful for experiments where the timing of the trials is important -
% such as in fMRI experiments where you have to get on with things
% if the subject doesn't respond in time. Here's an example of how to use it.
...
% Elapsed time is 5.003492 seconds.
% You pressed the "d" key after  0.27 seconds
% 'waitTill' can take in a second argument that holds the current time on the clock,
% as defined by the function GetSecs.
% This will be useful later when we're dealing with trial sequences and want to pause the program
% until the clock reaches a certain time.
%% Drawing text in real-world coordinates

% Often in experiments we put up text strings for instructions to the observer.
% We'll do that in a bit to instruct the observer to press a key to begin,
% and to tell the observer which keys to press for a response.

% We used the 'DrawText' function in Screen back in Lession 1.
% This draws a text string in a specified color with the upper-left corner at a specified location
% in pixel coordinates. Text parameters can be set using Screen functions like 'TextSize'.
...
% centers the text in real-world coordinates centered at the desired location.
% Here's some help on that function:
...
% We're ready to present a partially correlated moving stimulus that could be either
% moving upward or downward, get the subject's response, and provide feedback by flashing
% the fixation green (correct) or red (incorrect).
...
%choose either up or dowuun for the dot direction
